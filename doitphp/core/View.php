...
class View {
    ...
    protected function _parseViewFile($fileName = null) {
        ...
        //参数分析
        if (!$fileName) {
            $viewFileName = str_replace('_', DS, App::getControllerName()) . DS . App::getActionName();
        } else {
            $fileName     = str_replace('.', '/', $fileName);
            $viewFileName = ((strpos($fileName, '/') === false) ? str_replace('_', DS, App::getControllerName()) . DS : '') . $fileName;
        }
