 ...
 * @version $Id: App.php 3.1 2017-08-11 15:50:00Z tommy <tommy@doitphp.com> $
 * @package core
 * @since 1.0
 */
namespace doitphp;
    ...
    private static function _createWebApplication($routerInfo) {
        ...
        if (!isset($_app[$appId])) {
            ...
            if (strpos($controller, '_') !== false) {
                ...
                $nameSpaceArray = explode('_', strtolower($controller));
                $className      = ucfirst(array_pop($nameSpaceArray));
                $subNameSpace   = implode(DS, $nameSpaceArray);

                $className   = $subNameSpace . DS . $className;
                $controller  = str_replace(DS, '\\', $className);
            } else {
                $className   = $controller;
            }

            //分析controller文件的路径
            $controllerFilePath = BASE_PATH . '/controllers/' . $className . 'Controller.php';
 ...
 * 自动加载引导文件的加载
 */
App::loadFile(DOIT_ROOT . '/core/AutoLoad.php');
